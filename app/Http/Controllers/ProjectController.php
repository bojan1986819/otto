...
class ProjectController extends Controller
{
    ...
    public function phpgridManageProjects(){
        ...
        $g3 = new jqgrid();
        $opt["caption"] = "Awards és Festivals";
        $opt["datatype"] = "local"; // stop loading detail grid at start
        $opt["shrinkToFit"] = true;
        $opt["resizable"] = true;
        $g3->set_options($opt);
        ...
        $g3->select_command = "SELECT * FROM awards WHERE project_id = $id";
        $g3->table = "awards";
        ...
        $h["on_insert"] = array(function($data){
            $id = intval($_GET["rowid"]);
            $nowdate = date('y/m/d');
            $data["params"]["project_id"] = $id;
            $data["params"]["created_at"] = $nowdate;
        }, null, true);
        $h["on_update"] = array(function($data){
            $nowdate = date('y/m/d');
            $data["params"]["updated_at"] = $nowdate;
        }, null, true);
        ...
        $out_award = $g3->render("list3");

        return view('admin.projects', array('projects_output' => $out_projects, 'cast_output' => $out_cast, 'award_output' => $out_award));
    }
